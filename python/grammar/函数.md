...
### 3.2.2  闭包函数所引用的父函数中的局部变量是否具有全局变量的特点？
不具备，每个闭包函数对象的实例相互独立：
```python
def outer_func():
    loc_list = []
...
## 闭包是如何实现的？
&emsp;&emsp; python闭包中引用的自由变量实际存放在一个`Cell`对象中，当自由变量被闭包引用时，便将Cell中存放的自由变量的引用放入栈顶。
```python
def adder(x):
    def wrapper(y):
        return x + y
...
print(adder_obj.__closure__)
print(adder_obj.__closure__[0].cell_contents)
```
