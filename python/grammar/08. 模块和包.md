- [1. python中的模块、库、包有什么区别？](#1-python中的模块库包有什么区别)
- [2. `__all__`](#2-__all__)
  - [2.1 `__all__`的作用](#21-__all__的作用)
    - [2.1.1 总结](#211-总结)
    - [2.2.2 作用](#222-作用)
  - [2.2 `__all__`的一个应用：使用 `__all__`控制 `from xxx import *` 的行为](#22-__all__的一个应用使用-__all__控制-from-xxx-import--的行为)
  - [2.3 定义 all 需要注意的地方](#23-定义-all-需要注意的地方)
  - [2.4 实例验证](#24-实例验证)
    - [不带`__all__`](#不带__all__)
    - [带`__all__`](#带__all__)
- [3. `__init__.py`文件](#3-__init__py文件)
  - [3.1 `__init__.py`文件的作用是？](#31-__init__py文件的作用是)
  - [3.2  实例说明`__init__.py`的作用：一个普通的四则运算模块](#32--实例说明__init__py的作用一个普通的四则运算模块)
    - [不使用`__init__.py`](#不使用__init__py)
    - [使用`__init__.py`简化`import`操作](#使用__init__py简化import操作)
  - [3.3 `__init__.py`的设计原则](#33-__init__py的设计原则)
  - [参考文献](#参考文献)
- [4. import](#4-import)
  - [4.1 什么样的文件可以被`import`？](#41-什么样的文件可以被import)
  - [4.2 用`import`导入一个模块的时候，发生了什么？](#42-用import导入一个模块的时候发生了什么)
    - [4.2.1 总结](#421-总结)
    - [4.2.2 解释器是如何搜索模块文件的？](#422-解释器是如何搜索模块文件的)
      - [4.2.3 用`import`导入一个模块时，里面的代码会被执行吗？](#423-用import导入一个模块时里面的代码会被执行吗)
  - [4.3 当我们`import`一个包时，引入的是什么？](#43-当我们import一个包时引入的是什么)
  - [4.4   `import`模块时建议怎么做？](#44---import模块时建议怎么做)
  - [参考文献](#参考文献-1)
...
> 
**总结一下：**
> **`__init__.py`的作用**：`__init__.py`被用来标识一个文件夹是否是一个包(package)；
...
## 参考文献
1. (`__init__.py`的神奇用法)[https://zhuanlan.zhihu.com/p/115350758]




# 4. import
## 4.1 什么样的文件可以被`import`？
...
## 4.2 用`import`导入一个模块的时候，发生了什么？
### 4.2.1 总结
&emsp;&emsp; 在python中，**用`import`导入并非只是把一个文本文件插入到另一个文件。**导入其实是运行时的运算，程序第一次导入指定文件时，会执行以下三个步骤：
> ① 搜索模块文件
> ② 编译成位码（即pyc文件）
...
#### 4.2.3 用`import`导入一个模块时，里面的代码会被执行吗？
会被执行的，来看一个例子。
`mys_module.py`代码如下：
```python
def print_self():
...
## 参考文献
[深入 Python import 机制](http://sinhub.cn/2019/05/python-import-machinery-part-one/)
[python import导入时，发生了什么？](https://blog.csdn.net/weixin_45144170/article/details/104907793)
