...
- [2. `__all__`](#2-__all__)
  ...
  - [2.3 定义 `__all__` 需要注意的地方](#23-定义-__all__-需要注意的地方)
  - [2.4 实例验证](#24-实例验证)
    - [不带`__all__`](#不带__all__)
    - [带`__all__`](#带__all__)
- [3. `import`](#3-import)
  - [3.1 什么样的文件可以被`import`？](#31-什么样的文件可以被import)
  - [3.2 用`import`导入一个模块的时候，发生了什么？](#32-用import导入一个模块的时候发生了什么)
    - [3.2.1 总结](#321-总结)
    - [3.2.2 解释器是如何搜索模块文件的？](#322-解释器是如何搜索模块文件的)
      - [3.2.3 用`import`导入一个模块时，里面的代码会被执行吗？](#323-用import导入一个模块时里面的代码会被执行吗)
  - [3.3 当我们`import`一个包(模块)时，引入的是什么？](#33-当我们import一个包模块时引入的是什么)
  - [3.4   `import`模块时建议怎么做？](#34---import模块时建议怎么做)
- [4. `__init__.py`文件](#4-__init__py文件)
  - [4.1 `__init__.py`文件的作用是？](#41-__init__py文件的作用是)
  - [4.2  实例说明`__init__.py`的作用：一个普通的四则运算模块](#42--实例说明__init__py的作用一个普通的四则运算模块)
    - [不使用`__init__.py`](#不使用__init__py)
    - [使用`__init__.py`简化`import`操作](#使用__init__py简化import操作)
  - [4.3 `__init__.py`的设计原则](#43-__init__py的设计原则)
  - [参考文献](#参考文献)
