...
- [3. `subprocess`](#3-subprocess)
	...
	- [3.5 `subprocess.Popen`类](#35-subprocesspopen类)
	- [`subprocess.run()`和`subprocess.Popen`](#subprocessrun和subprocesspopen)
...
## 3.5 `subprocess.Popen`类


## `subprocess.run()`和`subprocess.Popen`
run是通过Popen实现的
run的阻塞的，Popen不是阻塞的
...
6. [subprocess --- 子进程管理](https://docs.python.org/zh-cn/3/library/subprocess.html)
7. [每周一个 Python 模块 | subprocess](https://zhuanlan.zhihu.com/p/91342640)
8. [https://zhuanlan.zhihu.com/p/348311219](https://zhuanlan.zhihu.com/p/348311219)