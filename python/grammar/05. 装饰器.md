...
- [3. 装饰器的原理是？](#3-装饰器的原理是)
  - [3.1 装饰器的基础一：python的函数可以像变量一样传递](#31-装饰器的基础一python的函数可以像变量一样传递)
  - [3.2 装饰器的基础二：在函数中定义函数 和 从函数中返回函数](#32-装饰器的基础二在函数中定义函数-和-从函数中返回函数)
  - [3.3 装饰器原理](#33-装饰器原理)
  - [4. 如何使用装饰器？](#4-如何使用装饰器)
- [4. 装饰器的参数](#4-装饰器的参数)
  - [4.1 `*args` 和 `**kargs` 是什么？](#41-args-和-kargs-是什么)
  - [4.2 一定要用`*args` 和 `**kargs`，直接写需要的参数名不行吗？](#42-一定要用args-和-kargs直接写需要的参数名不行吗)
    - [4.2.1 只修改`decorator_name装饰器`的形参形式](#421-只修改decorator_name装饰器的形参形式)
    - [4.2.2 修改`decorator_name装饰器`的形参形式，test()函数改为接收三个参数](#422-修改decorator_name装饰器的形参形式test函数改为接收三个参数)
  - [4.3 如何编写带参数的装饰器？如何调用？](#43-如何编写带参数的装饰器如何调用)
- [总结](#总结)
- [参考文献](#参考文献)
...
## 4.2 一定要用`*args` 和 `**kargs`，直接写需要的参数名不行吗？
