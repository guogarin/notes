...
- [五 `concurrent.futures`](#五-concurrentfutures)
  ...
  - [2. `concurrent.futures`主要包含哪些实用的工具？](#2-concurrentfutures主要包含哪些实用的工具)
    - [`concurrent.futures`如何确定进程池(线程池)的个数？](#concurrentfutures如何确定进程池线程池的个数)
...
> &emsp;&emsp; `concurrent.futures`提供了
...
## 2. `concurrent.futures`主要包含哪些实用的工具？
&emsp;&emsp; 库中包含了若干 执行器对象类`Executor Objects`：
**① `Executor`类**
...
### `concurrent.futures`如何确定进程池(线程池)的个数？
