...
- [三、 多线程](#三-多线程)
  ...
  - [3. `threading`](#3-threading)
    ...
    - [事件对象`Event`](#事件对象event)
      - [`Event`的作用](#event的作用)
      - [`Event`提供的方法](#event提供的方法)
      - [`Event`的使用实例](#event的使用实例)
- [四、协程(Coroutines)](#四协程coroutines)
  - [1. 什么是协程？](#1-什么是协程)
  - [2. 协程和线程](#2-协程和线程)
...
### 事件对象`Event`
#### `Event`的作用
&emsp;&emsp; 事件对象`Event`是线程之间通信的最简单机制之一：一个线程发出事件信号，而其他线程等待该信号。
...
> ① `await task1`
> ② 系统的调度，比如有某个子例程通过`asyncio.sleep(n)`主动将运行权交出；
> 
...
> ② `asyncio.sleep(n)` 会将运行权交出，此时那些在等待运行的 协程函数 会获得 运行权。
> ③ 
> 
...
运行结果：
```
spinner object: <Task pending name='Task-2' coro=<spin() running at :\code_practice\practice.py:19>>
     spin() is starting!
