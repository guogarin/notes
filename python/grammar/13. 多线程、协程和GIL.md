...
TODO:
### 2.2 协程和线程的对比
&emsp;&emsp; 协程是一种用户态的，不被操作系统内核所管理，完全由用户控制的，比线程更加轻量级的存在；
&emsp;&emsp; 一个进程可以由有多个线程，一个线程也可以有多个协程；
&emsp;&emsp; 用户态执行，性能大大提升：线程的切换由操作系统负责调度，协程由用户自己进行调度，因此减少了上下文切换，提高了效率。
&emsp;&emsp; 协程拥有自己的寄存器上下文和栈。协程调度切换时，将寄存器上下文和栈保存到线程的堆区，在切回来的时候，恢复先前保存的寄存器上下文和栈，直接操作栈则基本没有内核切换的开销，可以不加锁的访问全局变量，所以协程的上下文切换非常快。
...
&emsp;
## 8 `asyncio.Future`和 `asyncio.Task`
### 8.1 `asyncio.Future`
#### 8.1.1 `asyncio.Future`现状
官方文档是这么说的：
...
> 
#### 8.1.2 如何获取`asyncio.Future`对象？
> &emsp;&emsp; The rule of thumb is to never expose Future objects in user-facing APIs, and the recommended way to create a Future object is to call `loop.create_future()`. This way alternative event loop implementations can inject their own optimized implementations of a Future object.
> 
其它的见 [asyncio.Future的官方介绍](https://docs.python.org/3/library/asyncio-future.html#asyncio.Future)

### 8.2 `asyncio.Task`
#### 8.2.1`asyncio.Task`的几个要点
有如下几点需要注意:
> &emsp;&emsp; ① `asyncio.Task`是`asyncio.Future`的子类；
> &emsp;&emsp; ② 应该通过`asyncio.create_task(…)`获取`Task`对象，不应该自己实例化`Task`对象；
> &emsp;&emsp; ③ 当`asyncio.create_task(…)`返回一个`Task`对象时，该`Task`对象封装的协程函数会被自动调度执行；
> 
[asyncio.Task的官方介绍](https://docs.python.org/3/library/asyncio-task.html?%20gather#task-object)
#### 8.2.2 `Task`对象何时可以被取消？
Fluent Python 里的原文：
...
&emsp;
## 9. `asyncio.gather(*aws, return_exceptions=False)`
### (1) 函数介绍
**作用**： 
...
### 10.4 `asyncio.Semaphore`的使用场景？
&emsp;&emsp; `asyncio.Semaphore`可以限制并发的协程个数，有点类似于 线程池中的最大线程个数。
&emsp;&emsp; 比如在进行网络请求时，我们可以通过信号量来限制并发的请求数。

&emsp;
## 11 在线程中运行 : `asyncio.to_thread()`
### 11.1  `asyncio.to_thread(func, /, *args, **kwargs)`的作用
...
### 11.3 `asyncio.to_thread()`的替代品



### 11.4 `run_in_executor()`



&emsp;
## 12 等待原语(Waiting Primitives)
### 12.1 `asyncio.as_completed()`
#### (1) 函数介绍
...
&emsp;
## 14 `asyncio.sleep` 和 `time.sleep` 
### 14.1 `asyncio.sleep` 和 `time.sleep` 有何不同
...
### 14.3 `asyncio.sleep(0)`
&emsp;&emsp; `asyncio.sleep(0)`会使代码运行变慢，能不用就不用。



&emsp;
## 15 `yield from` 转 `async/await`
