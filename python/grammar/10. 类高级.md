...
- [1 python的魔术方法](#1-python的魔术方法)
  ...
  - [1.6 `__str__()` 和 `__repr__()`](#16-__str__-和-__repr__)
    ...
    - [1.6.2  `print()`调用的是哪个方法？](#162--print调用的是哪个方法)
      - [(1) 不显示指定](#1-不显示指定)
      - [(2) 显式指定](#2-显式指定)
    - [1.6.3 定义`__repr__()` 的标准做法](#163-定义__repr__-的标准做法)
    - [1.6.4 如果同时定义了`__str__()` 和 `__repr__()`，如何在指定调用哪个版本？](#164-如果同时定义了__str__-和-__repr__如何在指定调用哪个版本)
...
### 1.6.1 这两个方法的作用是？
&emsp;&emsp; 它俩控制的是实例的字符串表示。如果想改变一个实例的字符串表示，可重新定义它的 `__str__()` 和 `__repr__()` 方法。
...
### 1.6.3 定义`__repr__()` 的标准做法
&emsp; 前面定义`__repr__()`都是为了说明这两个魔术方法，对于`__repr__()`，标准做法是：
> 要让 `eval(repr(x)) == x` 为真。如果实在不能这样子做，应该创建一个有用的文本表示，并使用 < 和 > 括起来。比如：
> 
```python
>>> f = open('file.dat')
>>> f
...
### 1.6.4 如果同时定义了`__str__()` 和 `__repr__()`，如何在指定调用哪个版本？
显示指定即可：
```python
class Foo:
    def __str__(self):
