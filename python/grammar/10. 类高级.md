...
- [1、抽象基类(ABC, abstract class)](#1抽象基类abc-abstract-class)
  - [1.1. 基础](#11-基础)
    - [1.1.1 什么时候才应该使用 抽象基类？](#111-什么时候才应该使用-抽象基类)
    - [1.1.2 什么是抽象类？](#112-什么是抽象类)
    - [1.1.3 抽象基类有什么特点？](#113-抽象基类有什么特点)
    - [1.1.4 如何定义抽象基类？](#114-如何定义抽象基类)
    - [1.1.5 `collections.abc` 与 `abc`模块 是同一个东西吗?](#115-collectionsabc-与-abc模块-是同一个东西吗)
  - [1.2. 标准库中的抽象基类](#12-标准库中的抽象基类)
    - [1.2.1 标准库中的抽象基类在哪个模块中？](#121-标准库中的抽象基类在哪个模块中)
    - [1.2.2 抽象基类的 抽象方法(Abstract Methods) 和 `Mixin`方法(Mixin Methods) 是什么？](#122-抽象基类的-抽象方法abstract-methods-和-mixin方法mixin-methods-是什么)
    - [1.2.3 `collections.abc`模块中的ABC](#123-collectionsabc模块中的abc)
      - [1.2.3.1 `collections.abc`中包含的ABC](#1231-collectionsabc中包含的abc)
      - [1.2.3.2 `collections.abc`中包含的ABC 的分类](#1232-collectionsabc中包含的abc-的分类)
    - [1.2.4 `numbers`模块中的抽象基类](#124-numbers模块中的抽象基类)
    - [1.2.5 `issubclass()` 和 `isinstance()` 的用法](#125-issubclass-和-isinstance-的用法)
- [2. 子类化内置类型很麻烦(Subclassing Built-In Types Is Tricky)](#2-子类化内置类型很麻烦subclassing-built-in-types-is-tricky)
  - [2.1 继承内置类型会出现什么问题？](#21-继承内置类型会出现什么问题)
...
- [4. `property`属性](#4-property属性)
  ...
  - [4.2 使用`property`属性](#42-使用property属性)
    - [4.2.1 `property`属性 有哪些使用方法？](#421-property属性-有哪些使用方法)
...
- [5. 描述符(descriptor)](#5-描述符descriptor)
  ...
  - [5.7 总结](#57-总结)
  - [5.8 参考文献](#58-参考文献)
- [6. 运算符重载(Operator Overloading)](#6-运算符重载operator-overloading)
  - [6.1 Python的运算符重载有何限制吗？](#61-python的运算符重载有何限制吗)
  - [6.2 各个运算符对应的方法列表](#62-各个运算符对应的方法列表)
  - [6.3 一元运算符(Unary Operators)](#63-一元运算符unary-operators)
    - [6.3.1 有哪些一元运算符？它们对应的魔术方法是？](#631-有哪些一元运算符它们对应的魔术方法是)
    - [6.3.2 实现一元运算符需要注意算么？](#632-实现一元运算符需要注意算么)
    - [6.3.3 示例](#633-示例)
    - [6.3.4 `x` 和 `+x` 何时不相等](#634-x-和-x-何时不相等)
  - [6.4 `+`运算符](#64-运算符)
    - [6.4.1 解释器如何解析`+`运算符？](#641-解释器如何解析运算符)
    - [6.4.2 重载向量加法运算符`+`(Overloading `+` for Vector Addition)](#642-重载向量加法运算符overloading--for-vector-addition)
  - [6.5 乘法运算符`*`](#65-乘法运算符)
    - [6.5.1](#651)
  - [6.6 比较运算符(Comparison Operators)](#66-比较运算符comparison-operators)
  - [6.7 增量运算符(Augmented Assignment Operators)](#67-增量运算符augmented-assignment-operators)
    - [6.7.1 如何实现 增量运算符？](#671-如何实现-增量运算符)
    - [6.7.2 为 不可变类型 实现 增量运算符 时，需要注意什么？](#672-为-不可变类型-实现-增量运算符-时需要注意什么)
    - [6.7.3 如何对没有实现增量运算符的类调用 增量运算符，会发生什么？](#673-如何对没有实现增量运算符的类调用-增量运算符会发生什么)
  - [6.8 其它中缀运算符](#68-其它中缀运算符)
  - [6.9 运算符可以修改操作数吗？](#69-运算符可以修改操作数吗)
