...
&emsp;&emsp; 为这个类添加`__call__`方法即可，具体操作见，[类.md](类.md)中关于`__call__`的介绍。
...
&emsp;
&emsp;
# 24. `collections.abc` 与 `abc`模块 是同一个东西吗?
...
**总结：**
&emsp;&emsp; `collections.abc`：基于 `abc`模块定义了一些有用的的抽象基类，这些基类可以方便我们开发；
&emsp;&emsp; `abc`模块是用户用来自定义抽象基类的；
