...
- [3. 介绍一下python的`解包(unpacking)`机制](#3-介绍一下python的解包unpacking机制)
  - [3.1 什么是`解包(unpacking)`？](#31-什么是解包unpacking)
  - [3.2 将容器解包后赋予新的变量，这个新的变量被修改后，容器里面的元素会被修改吗？](#32-将容器解包后赋予新的变量这个新的变量被修改后容器里面的元素会被修改吗)
  - [3.3 如何理解`a, *b, c = [1,2,3,4]` ？](#33-如何理解a-b-c--1234-)
  - [3.4 带星号的表达式(starred expression)](#34-带星号的表达式starred-expression)
    - [3.4.1 什么是  带星号的表达式](#341-什么是--带星号的表达式)
    - [3.4.2 使用带星号表达式时需要注意什么？](#342-使用带星号表达式时需要注意什么)
    - [3.4.3 当要拆分的序列里已经没有元素留给 带星号表达式 时 会发生什么？](#343-当要拆分的序列里已经没有元素留给-带星号表达式-时-会发生什么)
  - [应用](#应用)
...
- [7. 切片(slice sequences)](#7-切片slice-sequences)
  - [7.1  什么样的类可以进行切片操作？](#71--什么样的类可以进行切片操作)
  - [7.2 切片 的时候如果下标越界会发生什么？](#72-切片-的时候如果下标越界会发生什么)
  - [7.3 切割出来的列表 和 原列表 有何联系？](#73-切割出来的列表-和-原列表-有何联系)
  - [7.4 进行切片操作时应该秉承什么样的原则？](#74-进行切片操作时应该秉承什么样的原则)
  - [7.5 切片 出现在 赋值符号的左侧](#75-切片-出现在-赋值符号的左侧)
  - [7.6 指定 步长(stride)](#76-指定-步长stride)
    - [7.6.1 指定步长有哪些应用？](#761-指定步长有哪些应用)
  - [7.7 如何为自定义的类提供 切片操作？](#77-如何为自定义的类提供-切片操作)
...
- [20. 命名元组(named tuple)](#20-命名元组named-tuple)
  ...
  - [20.3 `namedtuple`的使用场景](#203-namedtuple的使用场景)
...
## 20.3 `namedtuple`的使用场景
&emsp;&emsp; `namedtuple`适合用来构建只有少数属性但是没有方法的对象， 比如数据库条目
```python
import collections
