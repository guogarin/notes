...
&emsp;
## 6.4 `__getattr__(), __getattribute__(), __setattr__()`
### 6.4.1 `__getattr__()`
...
### 6.4.4 关于`__getattribute__()` 、 `__getattr__()` 和 `__setattr__()` 造成死循环的原因的 一点总结
...
### 6.4.5 如果一定要在`__getattribute__()` 、 `__getattr__()` 和 `__setattr__()`内部调用 本对象自己的属性，应该怎么做？
&emsp;&emsp; 为确保不会发生无限递归，使用`super()`来使用自己的属性最稳妥。

### 6.4.6 `__getattribute__()` 、 `__getattr__()` 和 `__setattr__()`这几个方法来自哪里？
