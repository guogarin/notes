...
# 6. Python的内置属性
## 6.1 `__dict__`属性
### 6.1.1 `__dict__`中存放的是什么？ 
&emsp;&emsp; `__dict__`是一个字典，其键为属性名，值为属性的值。

### 6.1.2 类本身的`__dict__` 和 类实例的`__dict__` 中存放的有何不同？
类本身的`__dict__` ：
...
&emsp;&emsp; 描述符就是一个“绑定行为“的对象属性，在描述符协议中，它可以通过方法充写属性的访问。我们或许经常看见描述符的作用描述中，有两个关键词“绑定行为”和“托管属性”，那到底是什么意思呢，这有一个通俗的解释：
...
而非数据描述 可能会被实例字典 `__dict__` 中定义的属性所覆盖。
...
&emsp;
## 10.5 描述符的应用
&emsp;&emsp; 描述符是 Python 中复杂属性访问的基础，它在内部被用于实现 property、方法、类方法、静态方法和 super 类型。
...
https://www.waynerv.com/posts/python-descriptor-in-detail/#%E6%8F%8F%E8%BF%B0%E7%AC%A6%E7%9A%84%E4%BD%9C%E7%94%A8

https://blog.csdn.net/qq_27825451/article/details/84566002
