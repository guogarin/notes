...
##### 4.2.2.1 `get_instance()`中为什么只进行了加锁，没进行解锁？
...
#### 4.2.3 最推荐的懒汉式单例(magic static )——局部静态变量
&emsp;&emsp; 这种方法又叫做 Meyers' Singleton，是《Effective C++》系列书籍的作者 Meyers 提出的。所用到的是在C++11标准中的Magic Static特性：
```cpp
class Singleton
{
public:
...
destructor called!
```
##### 4.2.3.1 此版本如何解决 线程安全 和 内存泄露 的问题？
**(1) 线程安全**
...
##### 4.2.3.2 相比于前面的实现，此版本好在哪？
(1) 不需要使用共享指针，代码简洁；
(2) 线程安全，且不用担心内存泄漏
##### 4.2.3.3 为什么要声明单例的引用 Single& 才能获取对象？
