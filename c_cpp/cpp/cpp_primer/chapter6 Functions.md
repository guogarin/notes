...
编译器会根据 传递的实参类型 判断想要的是哪个函数。


&emsp;
## 35.为什么需要重载？
更加方便程序员，几个功能相近，但是接受参数不一样的函数可以取同样的名字，这样就不需要记那么多名字了。
...
不算的，如果仅仅是返回类型不同，形参列表都一样，那么第二个函数的声明是错误的。


&emsp;
## 38. 有时候两个形参列表看起来是不一样的，但实际上是一样的
1.// 忽略了形参的名字，但实际上是一样的    
2.Record lookup(const Account &acct);    
3.Record lookup(const Account&); // parameter names are ignored    
4.
5.// 只是用typedef取了个别名而已，实际上形参是一样的。
6.typedef Phone Telno;    
7.Record lookup(const Phone&);    
8.Record lookup(const Telno&); // Telno and Phone are the same type    
...
5.Record lookup(Phone* const); //虽然是指针，但这是顶层const，因此重复声明了Record lookup(Phone*)


底层const会影响重载：
如果形参是指针或引用，则 通过区分它们指向的是常量对象还是非常量对象 可以实现函数的重载：
1.Record lookup(Account&); //函数作用于Account的引用    
2.Record lookup(const Account); //因为是底层const，所以是新函数，作用于常量引用    
3.     
4.Record lookup(Account*); //新函数，作用于指向Account的指针    
5.Record lookup(const Account*); //新函数，作用于指向常量的指针    


&emsp;
## 40.用 Account* ptr 调用下面两个函数，编译器会怎么处理？
1.Record lookup(Account*); //新函数，作用于指向Account的指针    
2.Record lookup(const Account*); //新函数，作用于指向常量的指针    
虽然两个函数都可以匹配成功(因为可以用非常量对象来初始化常量对象)，但编译器会优先选择最佳匹配的那个，即Record lookup(const Account*)


&emsp;
## 41. const_cast和重载
对于下面这个函数，实参和返回类型都const string的引用，我们可以用两个非常量的string来调用这个函数，但是返回的也是const string的引用：
1.//    实参和返回类型都const string的引用
2.const string &shorterString(const string &s1, const string &s2)    
3.{    
4.    return s1.size() <= s2.size() ? s1 : s2;    
5.}    
利用const_cast函数，重载shorterString函数，实参为非const string，最后也返回一个非常量string的引用：

1.// 重载shorterString，形参都是非常量
2.string &shorterString(string &s1, string &s2)    
3.{    
4.     // 利用const_cast将实参转换为const对象，然后调用另一个shorterString函数
5.        auto &r = shorterString(const_cast<const string&>(s1),    
6.        const_cast<const string&>(s2));    
7.    // 因为r是通过auto得到的，所以r的类型也是const string & ， 因此通过const_cast<string&>去掉底层const，返回一个非常量string对象
8.        return const_cast<string&>(r);    
9.}    
...
3.void print(double);//重载print函数    
4.void fooBar(int ival)    
5.{    
6.        bool read = false;	//新作用域：隐藏了外层的read函数    
7.        string s = read();	//错误：read是一个布尔值，而非函数    
8.        //不好的习惯：通常来说，在局部作用域中声明函数不是一个好的选择    
9.        void print(int);		//新作用域：隐藏了之前的print    
10.        print("Value:");		//错误：void print(const string &)被隐藏掉了    
11.        print(ival);			//正确：当前print(int)可见    
12.        print(3.14);			//正确：调用print(int);print(double)被隐藏掉了    
13.}    
print("Value:")报错的原因：
上面print函数的调用过程：在 void fooBar(int ival) 内声明的void print(int)隐藏了fooBar函数外的两个print函数，因此在fooBar函数内只有void print(int)可见，fooBar函数外的两个print函数都被隐藏了。
因此print("Value:")调用的是void print(int)函数，而字面值"Value:")是无法转换为int类型的，所以print("Value:")会报错
...
一旦某个形参被赋予了默认值，那它后面的所有形参都必须提供默认值
...
默认实参负责填补函数调用缺少的 尾部实参。也就是说，如果调用带默认实参的函数时，只能省略尾部的实参：
 如果第1个实参你省略了，那么后面的所有实参都必须省略；
 如果第2个实参你提供了非默认值，那么第1个实参你也必须提供非默认值；
下面来看几个例子：
1.typedef string::size_type sz;     
2.string screen(sz ht = 24, sz wid = 80, char backgrnd = ' ');    
3.
4.string window;    
5.window = screen();					 // equivalent to screen(24,80,' ')    
6.window = screen(66);				// equivalent to screen(66,80,' ')    
7.window = screen(66, 256); 			// screen(66,256,' ')    
8.window = screen(66, 256, '#'); 	// screen(66,256,'#')    
9.
10.window = screen(, , '?'); 		// 错误: can omit only trailing arguments
11.window = screen('?'); 			// calls screen('?',80,' ')    
对于 window = screen('?')：
...
&emsp;
## 46.默认实参和多次声明
 函数可以多次声明，但在同一作用域内，一个形参只能赋予一次默认实参；
 也就是说函数后续的声明只能为之前哪些没有默认值的形参添加默认实参，而且该形参右侧所有形参都必须有默认值。
1.// no default for the height or width parameters    
2.string screen(sz, sz, char = ' ');    
3.string screen(sz, sz, char = '*'); // 错误: 重复声明    
4.string screen(sz = 24, sz = 80, char); // 正确: 为前两个参数添加，默认实参，且最后一个参数前面已经有默认实参了(添加默认实参的形参的右侧所有形参都有默认值)


&emsp;
## 47. 默认实参的初始值
注意，局部变量不能作为默认实参！（因为默认实参是在编译时确定默认实参地址，在运行时取默认实参的值，而局部变量是自动变量，地址在运行时才能确定）
只要表达式的类型能转换为性擦所需的类型，该表达式就能作为默认实参
1.//    wd, def, and ht 的声明必须要出现在函数之外，也就是说不能为局部变量    
2.sz wd = 80;    
3.char def = ' ';    
4.sz ht();    
5.string screen(sz = ht(), sz = wd, char = def);    
6.string window = screen(); // calls screen(ht(), 80, ' ')    
7.void f2()    
8.{    
9.        def = '*'; // 变量def是全局变量，所以改变了默认实参的值
10.        sz wd = 100; // wd隐藏了外部同名的变量wd，但它没有改变函数screen()的默认值，因为它是局部变量    
11.        window = screen(); //def被改变了， 等同于调用screen(ht(), 80, '*')    
12.}    
...
&emsp;
## 48. 哪些变量不能作为函数的默认实参？
 局部变量（自动对象）不能作为函数的默认实参（因为默认实参是在编译时确定默认实参地址，在运行时取默认实参的值，而局部变量是自动变量，地址在运行时才能确定）
 类的非静态成员不能作为成员函数的默认实参


&emsp;
## 49.为什么局部变量不能作为函数的默认实参？
因为默认实参是在编译时确定默认实参地址，在运行时取默认实参的值，而局部变量是自动变量，地址在运行时才能确定。



&emsp;
## 50.内联函数有什么优点？为什么？ 
比普通的函数调用开销小，因为内联函数就是编译时把函数的定义替换到调用的，节省了函数调用开销。


&emsp;
## 51.内联函数为了提高性能，付出的代价是什么
内联通过把要调用的代码直接嵌入其中，省去一次函数调用的开销，这意味着代码会比较臃肿，这也是内联函数不宜过长的原因。
...
3.     int num1 = 1;    
4.     int num2 = 2;    
5.     int myNum = num1 + num2;	// 内联函数被替换了
6.}    
这意味内联函数可以节省很多开销
...
都知道C/C++的内存分为常量区、代码区、静态全局区、栈区、堆，其中函数代码存放在代码区，函数调用就发生在栈区里面，每次调用的时候会把当前函数的相关内容压入到栈里面处理寄存器相关的数据信息（所谓没有地址的右值很多情况就是指通过寄存器存储的数据）。然后，调用地址指向我们要执行的函数位置，开始处理函数内部的指令进行计算，当函数执行结束后，要弹出相关数据，处理栈内数据以及寄存器数据，这个过程也就是所谓的“函数调用开销”。


&emsp;
## 54. 内联函数（inline）和宏（#define）有何区别
处理方式： 宏做的是简单的字符串替换（注意是字符串的替换，不是其他类型参数的替换），而函数的参数的传递，参数是有数据类型的，可以是各种各样的类型。
是否用类型检查：内联函数要做参数类型检查，而宏不需要（仅仅是替换），从而内联函数相比宏而言更加安全
处理的时期：宏在编译之前（预编译）进行，即先用宏体替换宏名，然后再编译的，而函数显然是编译之后，在执行时，才调用的。因此，宏占用的是编译的时间，而函数占用的是执行时的时间。
空间占用：宏的参数是不占内存空间的，因为只是做字符串的替换，而函数调用时的参数传递则是具体变量之间的信息传递，形参作为函数的局部变量，显然是占用内存的。
　　开销：函数的调用是需要付出一定的时空开销的，因为系统在调用函数时，要保留现场，然后转入被调用函数去执行，调用完，再返回主调函数，此时再恢复现场，这些操作，显然在宏中是没有的。
...
4) 只将规模很小（一般5个语句一下）而使用频繁的函数声明为内联函数。因为在函数规模很小的情况下，函数调用的时间开销 可能相当于甚至超过 执行函数本身的时间，把它定义为内联函数，可大大减少程序运行时间。


&emsp;
## 56. 内联函数有什么缺点？
内联函数其实相当于给编译器一个提示，告诉它最好把这个函数在被调用处展开，省掉一个函数调用的开销（压栈，跳转，返回），它会使代码变得臃肿。


&emsp;
## 57.什么是constexpr函数
constexpr函数是指 能用于常量表达式 的函数。它和其它函数类似，不过有几个约束条件：
 函数的返回类型、所有的形参类型 都必须是 字面值类型；
 函数体内 有且仅有 一条return语句。
...
2.constexpr size_t scale(size_t cnt) {return new_sz()*cnt;}    
3.// 如果arg不是常量表达式，则scale(arg)不是常量表达式 
4.int arr[scale(2)];        //正确：scale(2)是常量表达式    
5.int i=2;                                //i不是常量表达式    
6.int a2[scale(i)];        //错误：scale(i)不是常量表达式    
...
它们可以在程序中多次定义，但多个定义必须完全一致，所以内联函数和constexpr函数一般都放在头文件中。
...
其中满足的是：
1.void f(); 			// 不满足，0个形参
2.void f(int); 		 	// 满足，5.6(double)可以转换为int，且形参只有一个
3.void f(int, int); 		// 不满足，有两个实参
4.void f(double, double = 3.14); // 满足，虽然有两个形参，但最后一个有默认实参
第三步，寻找最佳匹配，实参类型和形参类型最匹配的那个那个可行函数就是最佳匹配函数。在上面中，最匹配的是void f(double, double = 3.14)，因为实参5.6是double类型。


&emsp;
## 63.调用重载函数时，若有有多个形参是最佳匹配怎么办？
如果在检查了所有实参之后没有任何一个函数脱颖而出，则该调用是错误的，编译器将报告二义性调用的信息。（如果有多个都是最佳匹配，则报错）下面来看一个例子，
1.void f();    
2.void f(int);    
3.void f(int, int);    
4.void f(double, double = 3.14);    
5.f(42, 2.56);	// 调用的是哪个函数呢？    
最终入围最佳匹配函数是： 
1.void f(int, int);    			  // 只考虑第一个形参，它是最佳匹配
2.void f(double, double = 3.14);     // 只考虑第二个形参，它是最佳匹配
...
当两个同名函数一个接受int、一个接受short，则仅当传入实参为short 时才调用short，其它类型参数都会提升为int 或unsigned。有的时候，即使是一个很小的整数值，也会直接提升了int类型
1.void ff(int);    
2.void ff(short);    
3.ff('a'); // 'a'(char类型) 提升成了int，所以调用的是 f(int)    
值得注意的是，所有算术类型转换的级别都一样，例如 例如 从int向unsigned int的转换  并不比  int向double的转换 级别高，举个例子：
1.void manip(long);    
2.void manip(float);    
3.manip(3.14); 		// 错误: 二义性调用，3.14是double类型，它向long和float转换都是同样的级别，因此报错 
...
&emsp;
## 65.声明一个指针，指向bool lengthCompare(const string &, const string &);
声明函数指针的方法：用指针替换函数名即可（两端的括号不能少）：
1.// 声明了一个函数指针
2.bool (*pf)(const string &, const string &);     
3.// 声明了一个名为pf的函数，该函数返回 bool*
...
2.pf = lengthCompare; // pf 指向函数 lengthCompare    
3.pf = &lengthCompare; // 等价于pf = lengthCompare，取地址符（&）可有可无


&emsp;
## 67.声明了函数指针pf后，怎么利用它调用它指向的函数？
1.bool b1 = pf("hello", "goodbye"); // 调用lengthCompare    函数
2.bool b2 = (*pf)("hello", "goodbye"); // 等价于pf("hello", "goodbye")
3.bool b3 = lengthCompare("hello", "goodbye"); // equivalent call 
...
3.void (*pf1)(unsigned int) = ff; // 正确，指向了 ff(unsigned int)
4.void (*pf2)(int) = ff; // 错误: 两个ff 函数没有接受int形参的，虽然 有一个接受的是unsigned int，但不能类型转换
5.double (*pf3)(int*) = ff; // 错误: 两个ff 函数都没有返回值，此声明返回double
...
方法二：将 函数指针 作为形参
1.	//    将 函数类型 作为形参，因为他会自动的转换为指向函数的指针
...
3.void useBigger(const string&, const string&, Func); 
4.
5.// FuncP 是 指向函数的指针（函数指针）    
6.typedef bool(*FuncP)(const string&, const string&);    
7.void useBigger(const string&, const string&, FuncP); 

方法二：typedef + decltype
1.//  Func2 是函数类型        
2.typedef decltype(lengthCompare) Func2; // equivalent type    
3.void useBigger(const string&, const string&, Func2);    
4.
5.// FuncP 和FuncP2 是 指向函数的指针（函数指针）    
6.typedef decltype(lengthCompare) *FuncP2; // equivalent type    
7.void useBigger(const string&, const string&, FuncP2);    


&emsp;
## 71.可以返回函数吗？有什么折中的办法吗？
不能返回函数，但是可以返回函数指针。
1.using F = int(int*, int); // F 是函数类型，不是指针！    
2.using PF = int(*)(int*, int); // PF 是指针类型（函数指针）    
3.PF f1(int); // 正确: PF 是函数指针; f1 返回的是函数指针    
4.F f1(int); // 错误: F是函数 ; f1 并不能返回函数 
5.F *f1(int); // 正确: 虽然F是函数，但F* 是指针，因此fa返回的是函数指针    
不使用别名，我们也可以这么写：int (*f1(int))(int*, int)，只不过不那么好理解，现在我们来分析一下这个声明：


&emsp;
## 72.声明一个函数，这个函数 返回 指向int func(int*, int) 的函数指针，并 接受一个int实参
int (*f1(int))(int*, int)
...
1.typedef int (*B)(int);    // B是返回类型
2.B (*f)(int, int);    	 // 等同于 int (*(*f)(int,int))(int)
...
3.// 根据形参的取值，getFCN返回指向sumLength或largerLength的指针
4.decltype(sumLength) *getFcn(const string &);    // decltype返回的是函数类型，而不是指针，所以要加一个“*”
