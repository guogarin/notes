...
### 1.4.1 `static_cast`
#### (1) 作用
&emsp;&emsp; `static_cast`可以用于基本内置数据类型之间的转换，比如把`char`转成`float`、`double`转换成`long`等，这种内置类型之间的转换往往可以由隐式转换自动执行，而不需要人工特地去执行强制类型转换。由于转换结果可能存在截断性，这种转换的安全性要由开发人员来保证（一般不会出现严重程序运行错误，但是可能会出现逻辑错误），编译器可能会适当打印警告信息。
