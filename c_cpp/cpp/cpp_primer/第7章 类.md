...
### 105.2 相关总结
(1) `staitc成员` 是否占类空间？
&emsp;&emsp; `staitc成员`不占类对象的空间
(2) 成员函数是否占类空间？
&emsp;&emsp; ① **非虚函数** 不占类空间：
```cpp
class Demo{
public:
    Demo() {}
...
sizeof(d) : 1
```
&emsp;&emsp; ② **虚函数** 占8字节（一个指针的大小），包含虚函数的类，对象生成时，会在类对象当中插入一个指针，这个指针称做虚函数表指针，简称虚表指针(vPtr)。该指针指向一个虚函数表(简称虚表)，虚函数表中存储了虚函数的入口地址。基类当中有虚函数时，会产生该虚函数表；创建基类对象，对象中的vPtr会指向该表；调用虚函数时，是通过vPtr在此表当中寻找函数入口地址的。
```cpp
class Demo{
public:
    Demo() {}
