- [第十一章 关联容器](#第十一章-关联容器)
  - [关联容器 和 顺序容器 有何区别？](#关联容器-和-顺序容器-有何区别)
  - [标准库 提供了 哪些关联容器？](#标准库-提供了-哪些关联容器)
  - [map 和 set 有何区别？](#map-和-set-有何区别)
  - [如何定义一个 map 和 set ？](#如何定义一个-map-和-set-)
  - [编写一个程序，从标准输入读单词，统计每个单词出现的次数，并忽略: `"The", "But", "And", "Or", "An", "A","the", "but", "and", "or", "an","a"`](#编写一个程序从标准输入读单词统计每个单词出现的次数并忽略-the-but-and-or-an-athe-but-and-or-ana)
    - [遇到的问题](#遇到的问题)
      - [&emsp;1) 如何跳出 `while(cin >> word)`?](#1-如何跳出-whilecin--word)
  - [关联容器对它的 关键字 有何要求？](#关联容器对它的-关键字-有何要求)
      - [有序容器 对关键字 的要求](#有序容器-对关键字-的要求)
      - [无序容器 对关键字 的要求](#无序容器-对关键字-的要求)
  - [pair 类型](#pair-类型)
      - [定义在哪？](#定义在哪)
      - [如何定义一个 pair类型?](#如何定义一个-pair类型)
      - [不给pair类型指定初始值时会发生什么？](#不给pair类型指定初始值时会发生什么)
      - [pair类型 有哪些数据成员？](#pair类型-有哪些数据成员)
      - [pair类型 和其它 关联容器有何 关联？](#pair类型-和其它-关联容器有何-关联)
      - [make_pair()的作用是？原理是？](#make_pair的作用是原理是)
      - [](#)
  - [关联容器 有哪些顺序容器没有的 类型别名？](#关联容器-有哪些顺序容器没有的-类型别名)
      - [&emsp;下面的v1 - v5 分别是什么类型？](#下面的v1---v5-分别是什么类型)
  - [解引用关联容器的迭代器将得到什么？](#解引用关联容器的迭代器将得到什么)
  - [操作 map 时需要注意什么？](#操作-map-时需要注意什么)
  - [操作 set 时需要注意什么？](#操作-set-时需要注意什么)
  - [对 关联容器 使用 泛型算法 时需要注意什么？](#对-关联容器-使用-泛型算法-时需要注意什么)
  - [往map插入元素有哪几种方式？它们有何区别？](#往map插入元素有哪几种方式它们有何区别)
  - [如何判断 map 的 insert操作 是否成功？](#如何判断-map-的-insert操作-是否成功)
      - [如何理解上面的 `++ret.first->second;`](#如何理解上面的-retfirst-second)
      - [上面的 `word_count` insert操作的 返回值的类型是什么？](#上面的-word_count-insert操作的-返回值的类型是什么)
  - [multiset 和 multimap 的插入](#multiset-和-multimap-的插入)
      - [multimap 和 map 的插入 有何异同？](#multimap-和-map-的插入-有何异同)
  - [删除关联容器中 的元素](#删除关联容器中-的元素)
      - [有几种方式？](#有几种方式)
  - [关联容器的下标操作](#关联容器的下标操作)
      - [哪些 有 下标操作？为什么？](#哪些-有-下标操作为什么)
      - [下表操作有几种？ 它们有何区别？](#下表操作有几种-它们有何区别)
      - [使用 下标操作时需要注意什么？](#使用-下标操作时需要注意什么)
        - [修改](#修改)
      - [如果只是想知道 某个关键字 是否在 map或set 中，应该怎么写？](#如果只是想知道-某个关键字-是否在-map或set-中应该怎么写)
  - [关联容器的查找操作](#关联容器的查找操作)
      - [如要查找某一关键字是否在 容器中，推荐使用哪个方法？](#如要查找某一关键字是否在-容器中推荐使用哪个方法)
      - [如何统计一个关键字在 某关联容器中出现的次数？](#如何统计一个关键字在-某关联容器中出现的次数)
      - [如何查找 容器中 第一个 关键字 不小于(不大于)key 的元素？ 使用时需要注意什么？](#如何查找-容器中-第一个-关键字-不小于不大于key-的元素-使用时需要注意什么)
      - [如何在multimap 和 multiset中查找元素？](#如何在multimap-和-multiset中查找元素)
      - [在一个关联容器中查找元素的操作 列表](#在一个关联容器中查找元素的操作-列表)
  - [lower_bound() 、 upper_bound()、  equal_range()](#lower_bound--upper_bound--equal_range)
      - [它们的作用是？](#它们的作用是)
      - [lower_bound()、 upper_bound() 的原理](#lower_bound-upper_bound-的原理)
      - [使用 lower_bound()、upper_bound() 时需要注意什么？](#使用-lower_boundupper_bound-时需要注意什么)
  - [无序容器](#无序容器)
    - [无序容器的原理](#无序容器的原理)
      - [什么是 哈希表 和 桶？](#什么是-哈希表-和-桶)
      - [无序关联容器的 插入 过程是怎样的？](#无序关联容器的-插入-过程是怎样的)
      - [无序关联容器的 取值 过程是怎样的？](#无序关联容器的-取值-过程是怎样的)
      - [哈希函数 有何要求？](#哈希函数-有何要求)
      - [什么是哈希冲突？ 如何解决 哈希冲突？](#什么是哈希冲突-如何解决-哈希冲突)
      - [有可能多个元素映射到一个桶吗？如何解决这个问题？](#有可能多个元素映射到一个桶吗如何解决这个问题)
      - [无序容器的效率](#无序容器的效率)
      - [无序容器的 性能 和什么有关？](#无序容器的-性能-和什么有关)
      - [无序容器有何特点？](#无序容器有何特点)
      - [无序关联容器的 底层实现是怎样的？](#无序关联容器的-底层实现是怎样的)
      - [STL用什么来放桶节点 和 桶内的元素？](#stl用什么来放桶节点-和-桶内的元素)
      - [无序关联容器的时间复杂度](#无序关联容器的时间复杂度)
      - [无序容器支持的操作](#无序容器支持的操作)
        - [和有序容器一样的操作](#和有序容器一样的操作)
        - [无序容器 特有的操作](#无序容器-特有的操作)
      - [无序容器对关键字类型有何要求？为什么？](#无序容器对关键字类型有何要求为什么)
  - [有序关联容器的 底层实现](#有序关联容器的-底层实现)
      - [有序关联容器 的底层数据结构是什么？为什么选择它而不选择其它数据结构呢？](#有序关联容器-的底层数据结构是什么为什么选择它而不选择其它数据结构呢)
  - [有序关联容器 和 无序关联容器](#有序关联容器-和-无序关联容器)
      - [底层数据结构上的差异](#底层数据结构上的差异)
      - [元素组织上有何区别？](#元素组织上有何区别)
      - [性能上谁更好？为什么？](#性能上谁更好为什么)
      - [用哪个比较好呢？](#用哪个比较好呢)
  - [STL中的map 和 python的map 有何异同？](#stl中的map-和-python的map-有何异同)
  - [关联容器的底层原理](#关联容器的底层原理)
    - [1) set的底层原理？](#1-set的底层原理)
    - [2) map的底层原理](#2-map的底层原理)
