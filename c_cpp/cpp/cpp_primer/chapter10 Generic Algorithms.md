...
&emsp;
## 6 为什么算法不能（直接）添加或删除元素？
&emsp;&emsp; 因为算法是对迭代器进行操作的，而增加（删除）元素有可能会造成迭代器失效，所以不能。
...
#### (2) 谓词有哪几类？
&emsp;&emsp; 两类，分别是 **一元谓词(unary predicate)** 和 **二元谓词(binary predicate)**。所谓一元、二元指的是它们接受几个参数：
> 一元谓词接受一个参数；
> 二元谓词接受两个参数。
> 
#### (3) 谓词的作用？
...
### 9.8 如果只出现了`_3`，没有出现`_1`和`_2`，需要注意什么？
...
```
### 9.11 如何做到用下面的isShorter()函数对一个vector分别排 正序 和 倒序？
```cpp
...
```
### 9.12 对于那些不是占位符的参数，bind()函数是传值还是传引用？
&emsp;&emsp;默认情况下，那些不是占位符的参数，bind()函数是传值（即将其拷贝到bind返回的可调用对象中的）
### 9.13 下面的diamante有何问题？有的话怎么改？
```cpp
...
&emsp;
## 13 inserter、back_inserter 和 istream_iterator 有何区别？
