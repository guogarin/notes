...
### 8.1 出现问题的根本原因
在C/C++中，经常可能会涉及到一个无符号数与一个有符号数之间的运算。其实这个问题是关于C/C++语言中的整数自动转换原则：
> &emsp;&emsp; 当表达式中存在有符号类型和无符号类型时所有的操作数都自动转换为无符号类型。
> 
...
**① `wchar_t` 存在的原因：**
&emsp;&emsp; `char`是八位字符类型，最多能包含`256`中字符，许多的外文字符集所包含的字符数目超过`256`个（如中文），`char`型不能表示。
```cpp
wchar_t  bob = L'p';
```
