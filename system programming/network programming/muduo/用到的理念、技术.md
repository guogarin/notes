...
```
https://www.cnblogs.com/xf-said/p/6938651.html
https://blog.csdn.net/zhangxiao93/article/details/52792888
...
> (2) 写数据
> 
`shared_ptr`拥有对对象的引用计数，在对对象进行读写操作时，这个计数是`1`，当读数据时，我们创建一个新的智能指针指向原指针，这个时候引用计数加`1`。
```cpp
...
实际上：
假设一个线程读，一个线程写，当写线程进入到if循环中时，原对象的引用计数为2，分别为`tmpptr`和`g_ptr`，此时`reset()`函数将原对象的引用计数减1，并且`g_ptr`已经指向了新的对象（用原对象构造），这样就完成了数据的拷贝，并且原对象还在，只是引用计数变成了1。


TODO: 了解 assert()，本书库里用了好多
